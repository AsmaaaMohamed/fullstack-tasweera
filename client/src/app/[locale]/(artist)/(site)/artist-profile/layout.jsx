...
const sidebarItems = [
  {
    href: "/artist-profile/general",
    label: "معلوماتي العامة",
    labelEn: "My General Information",
  ...
  {
    href: "/artist-profile/email",
    label: "البريد",
    labelEn: "Email",
    icon: "/user/profile/sidebar/mail.svg",
  },
  {
    href: "/artist-profile/chats",
    label: "المحادثات",
    labelEn: "Chats",
    icon: "/user/profile/sidebar/chat.svg",
  },
  {
    href: "/artist-profile/working-hours",
    label: "الساعات",
    labelEn: "Working Hours",
    icon: "/user/profile/sidebar/streamline-cyber_email-2.svg",
  },
  {
    href: "/artist-profile/bank-info",
    label: "المعلومات البنكية",
    labelEn: "Bank Information",
    icon: "/user/profile/sidebar/guidance_bank.svg",
  },
  {
    type: "header",
    label: "الدعم",
    ...
    <div className="w-full bg-background dark:bg-[#363636]">
      <div className="max-w-7xl py-8 px-4 mx-auto sm:px-6">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          {/* Shared sidebar */}
          <aside
            className={`order-1 w-[320px] h-[690px] transition-transform duration-300 ease-in-out ${
              isMobileMenuOpen
                ? "fixed left-0 top-0 z-50 h-full translate-x-0 lg:relative lg:z-auto lg:translate-x-0 lg:h-[690px]"
                : "fixed left-0 top-0 z-50 h-full -translate-x-full lg:relative lg:translate-x-0 lg:z-auto lg:h-[690px]"
            }`}
          >
            <div className="rounded-xl lg:rounded-xl shadow-md bg-gray-100 dark:bg-[#323232] p-0 overflow-hidden pb-8 h-full lg:h-[690px]">
                  ...
                  if (item.action === "logout") {
                    return (
                      <button
                        ...
                        className="flex items-center gap-3 px-4 py-3 text-sm font-semibold text-gray-400 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#1C1C1D] w-full text-left"
                      >
                        {item.icon ? (
                          <Image
                            src={item.icon}
                  ...
                  if (item.action === "deleteAccount") {
                    return (
                      <button
                        key="deleteAccount"
                        onClick={handleLogout}
                        className="flex items-center gap-3 px-4 py-3 text-sm font-semibold text-red-600 hover:bg-gray-50 dark:hover:bg-[#1C1C1D] w-full text-left"
                      >
                        {item.icon ? (
                          <Image
                            src={item.icon}
