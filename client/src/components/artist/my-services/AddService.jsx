...
import { addArtistService } from "@/lib/artist-services";
import { toast } from "sonner";

export default function AddService({ open, onOpenChange, onSuccess }) {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const response = await addArtistService({
        name: formData.name,
      ...
      const errorMessage =
        ...
        (locale === "en"
          ...
          : "فشل إضافة الخدمة. يرجى المحاولة مرة أخرى.");
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl text-main text-center font-semibold">
            {t("add-new-service")}
          </DialogTitle>
        </DialogHeader>
